    ...
    <div class="flex flex-col bg-white m-auto rounded-2xl w-3/4 lg:flex-row lg:w-3/4 xl:w-2/4">
        ...
        <form action="{{ URL::to('register') }}" method="POST" novalidate
            ...
            <input type="text" id="name" name="name" placeholder="Name"
                class="h-10 px-4 mx-4 my-2 bg-[#F2F2F2] @error('name') border border-red-500 @enderror" value="{{ old('name') }}">
                @error('name')
                    <p class=" bg-red-500 text-white mx-4">{{ $message }}</p>
                @enderror
            <input type="text" id="username" name="username" placeholder="Username"
                class="h-10 px-4 mx-4 my-2 bg-[#F2F2F2] @error('username') border border-red-500 @enderror" value="{{ old('username') }}">
                @error('username')
                    <p class=" bg-red-500 text-white mx-4">{{ $message }}</p>
                @enderror
            <input type="email" id="email" name="email" placeholder="Email" class="h-10 px-4 mx-4 my-2 bg-[#F2F2F2] @error('email') border border-red-500 @enderror" value="{{ old('email') }}">
                @error('email')
                    <p class=" bg-red-500 text-white mx-4">{{ $message }}</p>
                @enderror
            <input type="password" id="password" name="password" placeholder="Password"
                class="h-10 px-4 mx-4 my-2 bg-[#F2F2F2] @error('password') border border-red-500 @enderror">
                @error('password')
                    <p class=" bg-red-500 text-white mx-4">{{ $message }}</p>
                @enderror
            <input type="password" id="password_confirmation" name="password_confirmation" placeholder="Confirm Password"
                class="h-10 px-4 mx-4 my-2 bg-[#F2F2F2] @error('password_confirmation') border border-red-500 @enderror">
